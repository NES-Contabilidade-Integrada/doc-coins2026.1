...
- **actors:** cadastro dos atores responsáveis pelos lançamentos.
...
- **apuracoes:** registra apurações contábeis vinculadas a contas de destino.
- **journal\_entry**: registra os lançamentos contábeis realizados por cada empresa.
- **journal\_entry\_line**: detalha as partidas de cada lançamento, indicando valores de débito e crédito.

O banco foi projetado para registrar lançamentos contábeis de diferentes empresas. O plano de contas (chart\_of\_account) organiza as contas de forma hierárquica, com cada conta podendo ser associada a um tipo (account\_types). Cada empresa (company) pode ter múltiplos lançamentos (journal\_entry), realizados por um ator (actors) e opcionalmente vinculados a uma apuração (apuracoes). Os lançamentos são detalhados em partidas (journal\_entry\_line) vinculadas ao plano de contas.
...
*  **amount** (tabela journal\_entry)
  ...
  Tipo: float com restrição CHECK (amount \> 0).
...
*  **is\_analytic** (tabela chart\_of\_account)
  ...
  Tipo: boolean com restrição implícita.
...
* **tipo** (tabela apuracoes)

  Tipo: inteiro com restrição CHECK (tipo IN (1, 2)).

  Finalidade: define o tipo da apuração, aceitando apenas os valores 1 ou 2.

  Relevância: garante que apenas tipos válidos de apuração sejam registrados, evitando inconsistências nos dados de fechamento contábil.
