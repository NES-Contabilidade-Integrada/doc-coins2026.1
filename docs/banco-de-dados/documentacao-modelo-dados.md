...
- **chart\_of\_account:** armazena os dados do Plano de Contas, incluindo hierarquia e identificação de contas analíticas e sintéticas.  
- **companies:** armazena as empresas cadastradas, incluindo informações básicas de identificação.  
- **journal\_entries**: registra os lançamentos contábeis realizados por cada empresa.  
- **journal\_entry\_lines**: detalha as partidas de cada lançamento, indicando valores de débito e crédito.
...
O Diagrama Entidade-Relacionamento (DER) foi modelado utilizando a ferramenta dbdiagram.io.  
Essa plataforma permite a escrita do modelo em DBML (Database Markup Language), que gera automaticamente o diagrama visual.  
Abaixo está o código DBML utilizado para a geração do DER:

// \---- chart\_of\_account \----  
Table chart\_of\_account {  
  id             text       \[pk\] // UUID gerado pela aplicação  
  code           text       \[not null\] // ex: 1.1.1.01  
  classification text       \[not null\]  
  description    text       \[not null\]  
  is\_analytic    int        \[not null, note: '1=Analítica | 0=Sintética'\]  
  created\_at     timestamp  \[default: \`CURRENT\_TIMESTAMP\`\]  
  updated\_at     timestamp  \[default: \`CURRENT\_TIMESTAMP\`\]  
  parent\_classification      text       \[ref: \> chart\_of\_account.code\] // autorrelacionamento por classification

  Indexes {  
    (code) \[name: 'idx\_chart\_of\_account\_code'\]  
  }  
}
...
// \---- journal\_entries \----  
Table journal\_entries {  
  id           text        \[pk\]  
  company\_id   text        \[not null, ref: \> companies.id\]  
  description  text  
  amount       decimal(14,2) \[not null\] // Valor total do lançamento  
  is\_compound  int         \[not null, default: 0, note: '1=composto | 0=simple'\]  
  created\_at   timestamp   \[default: \`CURRENT\_TIMESTAMP\`\]  
  updated\_at   timestamp   \[default: \`CURRENT\_TIMESTAMP\`\]

  Indexes {  
    (company\_id) \[name: 'idx\_journal\_company'\]  
  }  
}

// \---- journal\_entry\_lines \----  
Table journal\_entry\_lines {  
  id               text          \[pk\]  
  journal\_entry\_id text          \[not null, ref: \> journal\_entries.id\]  
  account\_id       text          \[not null, ref: \> chart\_of\_account.id\]  
  entry\_type       varchar       \[not null, note: "D=Débito | C=Crédito"\]  
  amount           decimal(14,2) \[not null, note: '\> 0'\]  
  created\_at       timestamp     \[default: \`CURRENT\_TIMESTAMP\`\]

  Indexes {  
    (journal\_entry\_id) \[name: 'idx\_jel\_entry'\]  
  }  
}
...
*  **entry\_type** (tabela journal\_entry\_lines)  
  ...
  Relevância: mantém a consistência contábil, evitando registros inválidos ou abreviações diferentes (como “Deb” ou “Cred”).

*  **is\_analytic** (tabela chart\_of\_account)

  Tipo: inteiro com restrição CHECK (is\_analytic IN (0,1)).

  Finalidade: distingue se a conta é analítica (1) — usada para lançamentos diretos — ou sintética (0) — usada apenas para agrupar contas.
...
* **parent\_classification** (tabela chart\_of\_account)
  ...
  Tipo: chave estrangeira (parent\_classification → chart\_of\_account.classification).

  Finalidade: estabelece um autorrelacionamento, permitindo que contas tenham subcontas associadas.

  Relevância: cria a hierarquia do plano de contas, refletindo a forma como a contabilidade organiza contas sintéticas e analíticas.
