...
#### Código - Código commitado e seguindo o padrão semântico definido, no repositório oficial.
- O nome da branch está seguindo a semântica estabelecida.
- Lint/format executado pelo autor antes do push.

#### Integração e Build - Pull Request aberto com descrição clara e referência ao ticket.
- Build do CI passa (compilação / lint).

#### Testes - Testes unitários relacionados à função estão implementados e funcionando.
- Cobertura mínima definida atendida (ex.: cobertura global ≥ 80% ou testes críticos cobertos).

#### Revisão e Aprovação - Revisão por pelo menos 1 revisor técnico aprovado.
